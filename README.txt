...
The module comes with five object plugins, one that copies all the datastreams
in an Islandora object to the top level of the Bag's 'data' directory, 
one that illustrates how to create an additional file to add to the Bag, one
that exports the FOXML for an Islandora object, one that creates an Archivematica
(http://archivematica.org) transfer Bag, and one that includes a PREMIS
preservation metadata file generated by the Islandora PREMIS module
(https://github.com/ruebot/islandora_premis).
